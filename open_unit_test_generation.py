...
def main(args):
    try:
        ...
        client, tokenizer = get_open_model(args.model, quantization='4bit')
        ...
        if multi_thread:
            ...
            for idx, row in df_input.iterrows():
                ...
                if not os.path.exists(wrapped_code_path):
                    ...
                    # resp = prompt_commercial_model(client, args.model, prompt, image_id="")
                    try:
                        resp = prompt_open_model(args.model, client, tokenizer, prompt)
                    except:
                        resp = "Fail to generate test cases, please try again later." 
                    print(resp)
                    save_txt(save_path, resp)
                    df_input.at[idx, col_name] = save_path
